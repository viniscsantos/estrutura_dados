...
void AVLSearchTree::insertAluno(NodeType*& tree, Aluno aluno, bool& isTaller){
    ...
    //O performRotatios vai ajustar o fatorB
...
void AVLSearchTree::printPreOrder(NodeType* tree) const {
    if(tree != NULL) {
        std::cout << tree->aluno.getNome() << "[ " << tree->aluno.getRa() << " ]";
...
void AVLSearchTree::performRotations(NodeType*& tree) const {
    NodeType* child;
    NodeType* grandChild; // Usado em rotação dupla
...
//Rotacionar para a direita
    if(tree->fatorB == -2) {
        ...
        switch (child->fatorB) {
        case -1 : //Simples para direita: Caso 1
        tree ->fatorB = 0;
        child->fatorB = 0;
        rotateToRight(tree);
         break;
        case 0: //Simples para direita: Caso 2 -> Remoções
        tree->fatorB = -1;
        child->fatorB = +1;
        rotateToRight(tree);
        break;
        case 1: //Rotação dupla
